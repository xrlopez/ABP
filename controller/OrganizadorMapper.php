...
class OrganizadorMapper {
  ...
  public function asignar(JuradoProfesional $jpro,$pinchos,Organizador $orga){
    foreach ($pinchos as $pincho) {
	  ...
	  //Añadido
	  //$stmt = $this->db->prepare("SELECT * FROM vota_pro WHERE FK_juradoProfesional_vota = ? AND FK_pincho_vota = ?");
	  //$stmt->execute(array($jpro->getId(), $pincho->getId()));
	  //$voto = $stmt->fetchAll(PDO::FETCH_ASSOC);
	  //if($voto == null){
		  //Lo de arriba no hace falta creo
  ...
  //Añadido
  /*public function votosPro($jProid, $idPincho){
	  $stmt = $this->db->prepare("SELECT * FROM vota_pro WHERE FK_juradoProfesional_vota = ? AND FK_pincho_vota = ?");
	  $stmt->execute(array($jProid->getId(), $idPincho->getId()));
	  $voto = $stmt->fetchAll(PDO::FETCH_ASSOC);
	  if($voto == null){
		$stmt = $this->db->prepare("INSERT INTO vota_pro VALUES(?,?,?,?)");
		$stmt->execute(array($jProid->getId(), $pincho->getId(),2,0));
	  }
  }*/
